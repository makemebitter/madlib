-- Here is how we created the dataset for param search on our local mac with 3 segment gpdb cluster
...
-- We tried to create the iris_data_param_search table by distributing by dist_key but it did not distribute the way we expected so we ended up just distributing randomly which prob does round robin and made sure that the data got distributed to all the segments
--
--create table iris_data_param_search as (select *, mod(__id__, 3)  as dist_key from iris_data_packed) distributed by (dist_key);
--
--param_search=# select gp_segment_id, count(*) from iris_data_param_search  group by gp_segment_id;
-- gp_segment_id | count
-----------------+-------
--             0 |    75
-- this should have returned three rows with seg ids as 0 , 1 and 2
